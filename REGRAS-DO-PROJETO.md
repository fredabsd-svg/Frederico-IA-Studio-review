...
- **Exceção da Etapa 1 de planejamento ([ADR-0037](docs/decisions/0037-etapa-1-de-planejamento-nao-inicia-a-trava-1-13.md)):** a Etapa 1 de uma fase é planejamento puro — ADRs e specs, sem código (ADR-0032 §D5). Enquanto a coluna "Evidência" da linha da fase no `status.md` contiver a marca literal **`somente-planejamento`**, os specs daquela fase podem permanecer "especificado", que é o estado verdadeiro deles. A marca some no primeiro PR de código da fase, porque é a mesma célula que esse PR reescreve para registrar a etapa fechada — não é um interruptor que se liga e esquece. Sem esta exceção, abrir uma fase obriga o spec novo a declarar implementação inexistente, violando a §1.1 para satisfazer a §1.13; foi o que aconteceu no `f7d1ab3` da Fase 7.
