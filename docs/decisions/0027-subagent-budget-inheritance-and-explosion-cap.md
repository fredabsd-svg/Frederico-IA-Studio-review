...
- 2026-08-06 — Etapa 4 PR 1 fecha a **infra de banco + tipos puros** desta ADR: `BudgetAllocation` + `SpentBudget` + `Budget::try_allocate` + `SubagentError` (6 variantes do D4) + `SubagentBudgetLedger` (livro de alocações testável em isolamento). `Run` em memória + storage ganham `subagent_count` + `depth` + `parent_run_id` + `spent`. Migração `0029_subagent_runs.sql` adiciona colunas em `runs` + nova tabela `subagent_runs`. **O `SubagentRunner` real fica pra Etapa 4 PR 2** (precisa de `ChatOrchestrator` no caminho de produção). Cortar na fronteira "infra pura vs. integração com orchestrator" é a mesma lição do PR #25 e do PR #34 (Etapa 3 em 2 PRs).
