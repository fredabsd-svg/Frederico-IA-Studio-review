...
Concretamente, o "negado por padrão" se sustenta em duas pernas separadas, e é importante não confundi-las: **(a)** todo código que respeita a convenção `HTTP_PROXY`/`HTTPS_PROXY` (a maioria das libs HTTP padrão — `requests`, `urllib`, `npm`, `pip`) só enxerga `127.0.0.1:PORT` no env e conecta lá; o proxy decide allow/deny pelo hostname do `CONNECT`/`Host:` header, **sem depender de DNS nenhum** (a resolução do destino real acontece do lado do proxy, não do filho). Essa perna **funciona e está testada** (Etapa 6). **(b)** código que resolve hostname por conta própria via `socket.getaddrinfo()` e decide o que fazer com o IP (inclusive quem ignora `HTTP_PROXY` e conecta raw) não passa pela perna (a) — a proposta original desta decisão era fechar essa segunda perna com um "nameserver interceptador" via `netsh` (Windows) redirecionando o resolver do filho pro proxy.

**Correção (2026-08-14, Etapa 7 da Fase 7):** a perna (b) **não funciona**. `netsh interface ip set dns name=Loopback source=static address=127.0.0.1` configura o servidor DNS da interface **Loopback Pseudo-Interface**, mas o Windows não consulta essa interface para resolver hostnames de verdade — ele usa os adaptadores de rede reais (Ethernet, Wi-Fi, etc.). Verificação manual fim-a-fim (Admin, `nslookup` sem servidor explícito, comparando resolução antes/durante/depois do `netsh`) confirmou: a resolução real do host não muda em nada com o intercept ativo. O mecanismo (`crates/security/src/dns_intercept.rs` + `dns_proxy.rs`) foi **removido por inteiro** — não ficou como capacidade parcial ou degradação declarada, porque não entregava proteção nenhuma em nenhum cenário (nem como Admin), só exigia privilégio e mexia na config de rede da máquina à toa (regra "capacidade incompleta é capacidade indisponível", `docs/status.md`).

**Consequência honesta:** "negado por padrão" vale só pra perna (a) — código que respeita `HTTP_PROXY`/`HTTPS_PROXY`. A perna (b) (DNS exfiltration via `getaddrinfo` + conexão direta) é uma lacuna **sem mitigação hoje**, documentada em `SECURITY.md` §"Rede" e `security-threat-model.md`. Fechar de verdade exige um mecanismo que force o tráfego de rede do filho a passar pelo proxy independente do que o filho faça — isso é o mesmo problema que o bypass por socket raw já documentado (item 1 do `SECURITY.md`), e a solução real é a mesma: firewall/filtro no nível de processo (Windows Filtering Platform / WDAC), roadmap de Fase 8+ (ver §Alternativas consideradas, item rejeitado "netsh no adaptador real").
...
### D5 — DNS passa pelo proxy, com resolução lazy (proposta — não implementada, ver D1)

Desenho original: o filho chama `socket.getaddrinfo("pypi.org")`; com intercepção via `netsh dns set`, o filho chamaria `127.0.0.1:53` (o próprio proxy), que resolveria via `tokio::net::lookup_host`, validando o hostname contra allowlist **antes** de resolver (para não virar vetor de enumeração), e devolveria IP se permitido — ou NXDOMAIN.

A regra "validar antes de resolver" continua correta **como desenho**: é o que fecharia DNS exfiltration, se o tráfego de fato passasse pelo responder. O problema (ver D1, correção 2026-08-14) é que ele nunca passa — `netsh` na interface Loopback não redireciona a resolução real do Windows. Um responder DNS mínimo real chegou a ser implementado (`crates/security/src/dns_proxy.rs`, RFC 1035 §QTYPE=A, testado ponta-a-ponta com tráfego UDP real) e removido junto com `dns_intercept.rs` — a lógica do responder estava correta, o problema é que nada roteava tráfego de verdade até ele. Se este mecanismo for retomado no futuro, o caminho é fechar primeiro o roteamento (WFP, não `netsh` na interface errada) antes de reconstruir o responder.
...
- `crates/security/src/dns_intercept.rs` — implementado na Etapa 6, **removido na Etapa 7** (2026-08-14) depois de verificação manual provar que não entrega proteção nenhuma. Ver D1 (correção) e §Pendências.
...
6. **`netsh` no adaptador de rede real (Ethernet/Wi-Fi), não na interface Loopback** (2026-08-14, depois de confirmar que a interface Loopback não funciona — ver D1). Rejeitado: sequestraria o DNS de **toda a máquina do usuário**, não só do processo sandboxed — qualquer outro programa rodando ao mesmo tempo (navegador, outros apps) também passaria a resolver via nosso proxy local enquanto o sandbox estivesse ativo. Blast radius incompatível com "sandbox isola o filho", vira "sandbox isola a máquina inteira". Se algum dia isso for retomado, o caminho correto é **Windows Filtering Platform (WFP)** — redireciona tráfego de rede por processo/PID, sem afetar o resto do sistema — ou rodar o filho numa rede isolada (namespace/VM), ambos roadmap de Fase 8+ e ambos exigem `unsafe`/privilégio de driver, fora do escopo de uma sessão de correção pontual.

## Pendências

- **DNS exfiltration não tem mitigação** (2026-08-14, era esperado que D1/D5 fechassem via `netsh` na interface Loopback — não funciona, ver D1). Filho que chama `socket.getaddrinfo(host)` diretamente e decide o que fazer com o IP (inclusive raw socket) resolve pelo DNS real do host, sem passar pelo proxy nem pela allowlist. Mesma família de problema do item abaixo (certificate pinning bypass) — mesma solução real (WFP/firewall por processo, Fase 8+).
...
- 2026-08-14 — **correção**: D1/D5 documentavam DNS interception (`netsh` na interface Loopback) como se fechasse DNS exfiltration. Verificação manual fim-a-fim (a primeira vez que alguém testou o caminho real, com um responder DNS de verdade do outro lado) provou que o mecanismo nunca funcionou — a interface Loopback não afeta a resolução real do Windows. `dns_intercept.rs` e o responder `dns_proxy.rs` (construído nesta mesma etapa) foram removidos por inteiro. "Negado por padrão" (D1) passa a valer só para código que respeita `HTTP_PROXY`/`HTTPS_PROXY` — DNS exfiltration via `getaddrinfo` direto é lacuna sem mitigação, mesma família do bypass por socket raw já documentado.
