...
| #24 | [`pr-fase-ligacao-etapa-5.md`](./pr-fase-ligacao-etapa-5.md) | Etapa 5 — nova crate `frederico-e2e` (14º membro, `publish = false`, path `crates/e2e/tests/`) com 5 testes E2E atravessando o caminho de produção **sem subir a casca Tauri** (consomem `frederico_app::build_chat_orchestrator` direto). Helper `build_orchestrator` em `tests/common/mod.rs` é o ponto único de montagem. 4 testes sempre rodam (`files.read`, degradação declarada, jail por conversa, `docs.generate` com `FakeWorker`); 1 teste `#[ignore]` (`docs.generate` com `DocumentWorkerLauncher` real — único que atravessa o Python e gera `.docx` de verdade). `testing-strategy.md` promovido a `parcialmente implementado` com 2 adições (fronteira do que os E2E cobrem + regra da composição compartilhada). `verify-external.ps1` Step 7 ativa o teste de worker real. |
| #25 | [`pr-fase-ligacao-patch-allowed-paths.md`](./pr-fase-ligacao-patch-allowed-paths.md) | **Etapa 5.X (patch-allowed-paths)** — patch de segurança no caminho de produção. A Etapa 3 da Fase 5 (set/2025) deixou a barreira de path safety do `docs.generate` **desligada E quebrada** no caminho de produção (dois defeitos empilhados que se escondiam mutuamente: vetor vazio no construtor + `validate_against_allowlist` fail-open **e** bug do verbatim `\\?\` latente — o fail-open bypassava a função inteira, então o verbatim nunca foi exercitado). Fix: `WorkerToolDispatcher` recebe allowlist por chamada (resolve do `ctx.jail.root_canonical()`); `Jail::resolve_allowing_nonexistent` como barreira primária; mitigação symlink explícita (rótulo "parcial", TOCTOU documentado); `check_path` fail-closed como defesa em profundidade; `Jail::root_canonical` getter exposto; 6 cenários de `path_safety` no `document-kits`. **Comportamento visível mudou:** `docs.generate` agora recusa `output_path` absoluto (e.g. `C:\...`); `docs.inspect` agora recusa arquivo fora do workspace da conversa (leitura sem jail seria porta lateral pra ler `~/.ssh/id_rsa`). Pendência 1 do `process-architecture.md` fechada; pendência 5 nova (escrita segura no worker Python — `O_NOFOLLOW` / `O_CREAT|O_EXCL` no `open` dos handlers). Lição 1: defaults fail-open escondem que o mecanismo nunca funcionou. Lição 4: `cargo test --workspace --lib` não roda integration tests — sem `cargo test --workspace` completo, o PR teria sido aberto com 4 testes quebrados. **5 de 6 etapas da Fase de Ligação fechadas.** |
...
| **Etapa 5.X** — patch de segurança (path safety do `docs.generate`/`docs.inspect` ligado de verdade; `Jail::resolve_allowing_nonexistent` como barreira primária) | **fechada** (PR #25) | — | nenhuma |
