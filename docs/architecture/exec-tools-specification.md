...
> Spec criado na Etapa 1 da Fase 7 (PR de planejamento, 2026-08-08), atualizado na **Etapa 4** (2026-08-08) e na **Etapa 7** (2026-08-14, PR `fase-7-etapa-7-exec-shell`). **Atualizado em 2026-08-16 pelo [ADR-0037](../decisions/0037-exec-shell-fora-do-catalogo.md): `exec.shell` saiu do catálogo** — a allowlist de comandos que a justificava é contornável por qualquer separador do `cmd.exe` (`echo x & <qualquer coisa>` passa pelos dois gates), e 7 dos 9 comandos da allowlist são binários MSYS2 que morrem sob o rótulo de integridade baixa. O que a ferramenta precisa entregar pra voltar está no ADR-0037 §D5. A seção "`FilesExecShellTool`" abaixo descreve o código que **existe** em `shell.rs` mas **não é registrado** — leia como especificação pendente, não como capacidade viva. Especificação das 3 ferramentas `exec.*` — 2 implementadas e no catálogo (`exec.python`, `exec.node`, Etapa 4) e 1 fora do catálogo (`exec.shell`, Etapa 7 — a numeração real das etapas divergiu do planejamento original da Etapa 1; a rede do sandbox fechou primeiro como Etapa 6/6+1, `exec.shell` fechou depois como Etapa 7; ver nota em `windows-sandbox-design.md`). **Etapa 4 fechou**: `FilesExecPythonTool` + `FilesExecNodeTool` in-process em `crates/tool-registry/src/exec/`; integração com `SecurityJailResolver` (Etapa 2) + `RuntimeRegistry` (Etapa 3) via `Arc<...>` na `FilesExecToolBase`; **per-invocation Job Object**; `wait_with_timeout` dentro do `collect_output`; aprovação obrigatória via `validate_tool_call` Passo 9. **Etapa 7 fechou**: `FilesExecShellTool` (`shell.rs`, ~250L) + `frederico_security::exec_patterns` (denylist + allowlist, `~180L`). Diferenças do plano original documentadas na seção "`FilesExecShellTool`" abaixo — a mais relevante é que a denylist/allowlist são aplicadas **incondicionalmente** (não gateadas por `PermissionSet::terminal`), porque o `ToolContext` que chega no `Tool::execute` não carrega o `PermissionSet` da run (isso é responsabilidade do `validate_tool_call` no `RunExecutor`, que hoje só implementa o Passo 5 para `file_read` — os demais eixos, incluindo `terminal`/`python`/`node`, são bumpados no `PermissionSet` mas ainda não lidos por nenhum gate em runtime; ver `crates/tool-registry/src/validate.rs`).
...
### `FilesExecShellTool` (Etapa 7 — **fora do catálogo desde o ADR-0037**)

> **Leia esta seção como especificação pendente.** O código existe em `crates/tool-registry/src/exec/shell.rs`, mas `build_default_exec_tools` **não** o registra desde 2026-08-16 ([ADR-0037](../decisions/0037-exec-shell-fora-do-catalogo.md)). A allowlist descrita abaixo foi medida e **não é uma barreira**: `is_allowed` valida só o primeiro token e `build_args` entrega o comando inteiro pro `cmd.exe /c`, que interpreta `&`, `&&`, `||` e `|` — então `echo marcador & ver` executa os dois, com `ver` sozinho sendo recusado. Além disso, 7 dos 9 comandos da allowlist (`ls`, `cat`, `head`, `tail`, `grep`, `wc`, `pwd`) são binários MSYS2 que morrem sob o rótulo de integridade baixa com `0xC0000022`. Os 3 requisitos pra ferramenta voltar estão no ADR-0037 §D5.

Implementado (mas não registrado) em `crates/tool-registry/src/exec/shell.rs` + `frederico_security::exec_patterns`:
...
| `crates/e2e/tests/e2e_exec_shell_out_of_catalog.rs::exec_shell_is_not_in_default_catalog` (ADR-0037) | `build_default_exec_tools` não devolve `exec.shell` — com controle positivo (`exec.python`/`exec.node` continuam lá), senão passaria com o catálogo vazio. Substitui `e2e_exec_shell_denylist.rs::rm_rf_is_rejected_by_denylist` e `e2e_exec_shell_allowlist.rs::echo_allowlisted_command_executes`, apagados pelo ADR-0037 §D4 (REGRA §3.4 exige ADR pra apagar teste nomeado). |
| `crates/e2e/tests/e2e_exec_shell_out_of_catalog.rs::allowlist_that_justified_the_tool_is_still_defeated_by_cmd_separators` (ADR-0037) | Fixa a **razão** da remoção. Quando o `exec_patterns` aprender a recusar separadores de shell, este teste falha — e a falha é o sinal de que o ADR-0037 §D5 pode ser reaberto, não um defeito. |
