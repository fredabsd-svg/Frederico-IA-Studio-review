...
| Filho faz `rm -rf /` no host | **não** (dentro do sandbox: sim) | O filho roda sob usuário limitado + Restricted Token, mas `rm -rf` no workdir (que é o workspace) **funciona** (é o diretório do usuário). Defesa: `exec.python`/`exec.node` requerem aprovação por invocação. **A denylist do `exec.shell` não conta mais como defesa** — a ferramenta saiu do catálogo no ADR-0037, e a denylist nunca foi barreira (substring literal, contornável por `rm -r -f` e por qualquer separador do `cmd.exe`). |
...
| Etapa 7 — `exec.shell` + allowlist de rede via perfil | **fechada** (PR `fase-7-etapa-7-exec-shell`, 2026-08-14; `exec.shell` saiu do catálogo em 2026-08-16 pelo [ADR-0037](../decisions/0037-exec-shell-fora-do-catalogo.md) e voltou no mesmo dia pelo [ADR-0044](../decisions/0044-exec-shell-com-resolucao-propria-de-programa.md), Etapa 2b da Fase 8. A parte de `network_allowlist` via perfil TOML nunca foi tocada) | Fase 7 `concluída` | Etapa 2 + Etapa 4 + Etapa 6+1 | `FilesExecShellTool` + `frederico_security::exec_patterns`: **o `cmd.exe` não resolve programa** — builtin via `cmd.exe /d /v:off /c`, executável do `System32` por caminho absoluto com spawn direto; metacaracteres recusados antes do spawn; lista fechada de 11 programas read-only (ver `exec-tools-specification.md`). `PermissionSet.network_allowlist` novo campo, carregado do perfil TOML user∩project na casca |
...
| 7 | `crates/e2e/tests/e2e_exec_shell_out_of_catalog.rs::exec_shell_is_not_in_default_catalog` + `::allowlist_that_justified_the_tool_is_still_defeated_by_cmd_separators` (ADR-0037 §D4 — substituem os 2 E2E de denylist/allowlist, apagados) | mesmo arquivo | — |
