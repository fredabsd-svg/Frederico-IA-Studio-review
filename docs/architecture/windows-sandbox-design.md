...
| Etapa 7 — `exec.shell` com denylist/allowlist + allowlist de rede via perfil | **parcialmente revertida** (PR `fase-7-etapa-7-exec-shell`, 2026-08-14; `exec.shell` **fora do catálogo** desde 2026-08-16, [ADR-0037](../decisions/0037-exec-shell-fora-do-catalogo.md) — a allowlist de comandos é contornável por separador do `cmd.exe`. A parte de `network_allowlist` via perfil TOML **continua valendo**) | Fase 7 volta a `em andamento`; requisitos de retorno no ADR-0037 §D5 | Etapa 2 + Etapa 4 + Etapa 6+1 | `FilesExecShellTool` + `frederico_security::exec_patterns` (denylist+allowlist sempre ativas, não gateadas por `PermissionSet` — ver `exec-tools-specification.md`); `PermissionSet.network_allowlist` novo campo, carregado do perfil TOML user∩project na casca |
