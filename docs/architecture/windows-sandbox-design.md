...
| **Job Object** | Tree-kill garantido (mesmo em `kill -9`), limite de memória por processo e total | `exec.python`, `exec.node` | `crates/security/src/windows/job_object.rs` (novo, Etapa 2) |
| **Restricted Token** | Descarte de 6 privilégios elevados (SeDebug, SeBackup, SeRestore, SeTakeOwnership, SeLoadDriver, SeShutdown) | `exec.python`, `exec.node` | `crates/security/src/windows/restricted_token.rs` (novo, Etapa 2) |
| **Env zeroed + allowlist** | Fecha `I1` do threat model (env leak) | `exec.python`, `exec.node` | `crates/security/src/env_filter.rs` (novo, Etapa 2) |
| **Proxy local de rede** | Rede do sandbox passa por allowlist + log visível, **só pra código que respeita `HTTP_PROXY`/`HTTPS_PROXY`** (fecha SSRF reverso; **não** fecha DNS exfiltration — ver nota abaixo) | `exec.python`, `exec.node`, quando precisam de rede (pip install, npm install) | `crates/security/src/network.rs` (Etapa 6 da Fase 7) |
...
**DNS interception tentado e descartado (2026-08-14).** A Etapa 7 tentou fechar o vetor de DNS exfiltration com um `netsh interface ip set dns name=Loopback source=static address=127.0.0.1` + um responder DNS real do outro lado da porta 53 (`crates/security/src/dns_proxy.rs`, removido). Verificação manual fim-a-fim (Admin, `nslookup` sem servidor explícito) provou que o Windows **não** consulta a interface Loopback pra resolver DNS de verdade — ele usa os adaptadores de rede reais (Ethernet/Wi-Fi). O mecanismo não entregava proteção nenhuma, exigia Admin, e mexia na config de DNS da máquina do usuário à toa. `crates/security/src/dns_intercept.rs` e `dns_proxy.rs` foram removidos por inteiro (regra "capacidade incompleta é capacidade indisponível", `docs/status.md`). **DNS exfiltration continua um vetor aberto** pra qualquer filho que resolve hostname por conta própria (`socket.getaddrinfo` + conexão direta) — só o código que respeita `HTTP_PROXY`/`HTTPS_PROXY` (a maioria das libs HTTP padrão) passa pelo deny-by-default. Ver ADR-0033 §D1 (correção) e `SECURITY.md` §"Rede".
...
`exec.shell` foi tentado na Etapa 7 e descartado (2026-08-14) — não está no catálogo, sem linha nesta tabela. Ver §"Etapas da Fase 7" abaixo.
...
| Filho exfiltra via DNS | **não** | Tentativa de interceptar DNS via `netsh interface ip set dns name=Loopback` **descartada** (2026-08-14) — verificação manual fim-a-fim provou que o Windows não consulta a interface Loopback pra resolver DNS de verdade (usa os adaptadores reais). Filho que chama `socket.getaddrinfo(host)` diretamente resolve pelo DNS real do host, fora do proxy. **Lacuna documentada, sem mitigação** — só código que respeita `HTTP_PROXY`/`HTTPS_PROXY` passa pelo deny-by-default. |
| Filho lê `SAM` (Windows Security Account Manager) | **sim** | SeBackup removido (Restricted Token, ADR-0036 D4). |
| Filho faz `rm -rf /` no host | **não** (dentro do sandbox: sim) | O filho roda sob usuário limitado + Restricted Token, mas `rm -rf` no workdir (que é o workspace) **funciona** (é o diretório do usuário). Defesa: `exec.python` requer aprovação por invocação (não há `exec.shell` — tentado e descartado, 2026-08-14). |
...
| Etapa 6 — Rede do sandbox (proxy local) | **concluída** (PR #51, 2026-08-13, ADR-0033) | Etapa 6+1 | Etapa 2 | `crates/security/src/network.rs` + `DbNetworkAuditSink`, deny-by-default (só código que respeita `HTTP_PROXY`), HTTPS via `CONNECT` sem MITM. Tentativa de DNS intercept (`dns_intercept.rs`) descartada em 2026-08-14 (não funcionava) |
...
| Etapa 7 — allowlist de rede via perfil; `exec.shell` tentado e descartado | **concluída** (PR `fase-7-etapa-7-exec-shell`, 2026-08-14) | — (Fase 7 fecha) | Etapa 2 + Etapa 4 + Etapa 6+1 | `PermissionSet.network_allowlist` novo campo, carregado do perfil TOML user∩project na casca. `FilesExecShellTool` + `frederico_security::exec_patterns` foram implementados e **removidos no mesmo dia** — bypass via operadores de shell na v1, binários MSYS2 incompatíveis com Low Integrity na correção tentada; ver `exec-tools-specification.md` §"`exec.shell` (descartado)" |
...
- [`docs/architecture/exec-tools-specification.md`](./exec-tools-specification.md) — `exec.python`, `exec.node` (Etapa 4); `exec.shell` tentado e descartado (Etapa 7)
