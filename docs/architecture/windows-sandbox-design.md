...
| Etapa 3 — Runtimes embutidos | **concluída** (PR `fase-7-etapa-3-runtimes-embutidos`, 2026-08-08) | Etapa 4 | nenhuma | `crates/runtimes/` (16º membro do workspace) com Python 3.12.4 + Node 20.16.0 portáteis, `RuntimeRegistry::bootstrap_all` idempotente (cache hit >5x mais rápido), SHA-256 + size pinned no código (verificados via download em 2026-08-08), 5 testes de regressão (3 com rede: `python_env_vars_do_not_include_user_paths`, `bootstrap_twice_is_noop`, `corrupted_manifest_triggers_redownload`; 1 sem rede: `offline_returns_error_for_missing_runtime`; 1 com rede: `node_env_vars_do_not_include_user_paths`) |
...
| Etapa 5 — `files.write` / `files.edit` / `files.list` | **concluída** (PR `fase-7-etapa-5-files-write-list`, 2026-08-08, Etapa 5 v1) | Etapa 6 | Etapa 2 | `FilesWriteTool` + `FilesEditTool` + `FilesListTool` com Jail + atomicidade + backup + audit. **Etapa 5+ (path safety) fechada em 2026-08-10** (PR `fase-7-etapa-5-token-restrito`) — `Mandatory Label\Low` no workdir + `TokenIntegrityLevel=Low` no child + `RestrictedToken` aplicado via raw `CreateProcessAsUserW`. Reativou `exec.python`/`exec.node` no `ToolRegistry` com `python/node: None → Sandboxed`. 3 tests E2E verdes (`child_cannot_write_outside_workspace`, `exec_python_simple_hello_world`, `wall_clock_kills_long_running_process`). |
