...
> Última verificação: 2026-08-01. Reflete a **Fase 5** (Documentos) — `WorkerManager` redesenhado como ator (Etapa 2A, ADR-0015 + ADR-0016), transporte real sobre named pipes do Windows via Tokio (Etapa 2B, ADR-0017), `WorkerManager::spawn_external` (Etapa 2B continuação), 7 handlers reais do `document-worker` Python (Etapa 2B+X, ADR-0018; 6 originais + `ocr.run` da Etapa 2B+Y), `docs.generate` consumindo o `WorkerHandle::invoke` (Etapa 3, `DocsGenerateTool` + `ToolManifest::allowed_paths`), `docs.inspect` round-trip `.docx`/`.xlsx` (Etapa 4), `PdfProKit` real (Etapa 5 PR 2) + auditoria estrutural bloqueante do §19.4 via `pdf.audit` no `document-worker` v0.4.0 (Etapa 5 PR 3) — agora **8 handlers** no total. **`WorkerManager::spawn_external`** + `WorkerHandle::invoke` (Etapa 2B continuação) com handshake `READY <pipe_name>` (ADR-0017 §Decisão 2 — inversão do handshake) + 7 testes E2E em `tests/external_doc_worker.rs` (NAO `#[ignore]`, rodam em CI via `scripts/verify-external.ps1`) — bootstrap estendido instala Python 3.12.7 + pywin32 + 7 libs Python (`python-docx`, `openpyxl`, `reportlab`, `pdfplumber`, `pikepdf`, `pypdfium2`, `fonttools`) + Tesseract 5.4 + tessdata (por/eng/osd) + 4 TTFs (~386 MB total, cacheado por `actions/cache@v4` no `.github/workflows/ci.yml`; `LibSentinels` em `bootstrap.ps1` cobre as 7 libs com hard-fail D-FAIL-1). Fontes "Tinta e Latao" (Adobe Source Sans 3 + Source Serif 4) embutidas no PDF, detectadas via `font_status` no `worker.hello` e `worker.pong`. **Auditoria bloqueante do §19.6** (Etapa 5 PR 3, ADR-0021 D-PDF5): `PdfProKit::render` chama `pdf.audit` após `pdf.write`; falha mapeia pra `KitError::AuditFailed` e artefato NÃO é entregue (sem interruptor). `pikepdf` valida 5 checks baseline (abertura, n_pages >= 1, DocInfo populado, fontes embutidas, sem cifragem/referências externas) + 4 A-2B opt-in (OutputIntent com sRGB ICC v2 gerado localmente, XMP `pdfaid:part=2` + `pdfaid:conformance=B`, sem JavaScript). sRGB ICC v2 (D-PDF6) gerado por `tools/generate_srgb_icc.py` (SHA-256 pinado em `bootstrap.ps1`). **Tagged PDF NÃO é verificado** — A-2B (básico) não exige; v1 declara apenas nível B (PDF/A-2A fora de escopo declarado). **Schema bump `SpecVersion` 0.2.0 → 0.3.0** com `DocumentMetadata.pdfa: Option<PdfaSpec>` (opt-in PDF/A-2B, backward-compat). O detalhe da implementação vive em [`docs/modules/process-architecture.md`](../modules/process-architecture.md) e [`docs/modules/document-kits.md`](../modules/document-kits.md) (atualizado na Etapa 5 PR 3).
