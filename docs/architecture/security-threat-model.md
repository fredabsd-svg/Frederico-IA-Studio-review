...
| Filho faz `rm -rf /` no host | **dentro do sandbox: sim (afeta apenas workdir)** | `exec.python`/`exec.node` requerem aprovação por invocação (ADR-0034). A denylist do `exec.shell` **não** conta: a ferramenta saiu do catálogo no [ADR-0037](../decisions/0037-exec-shell-fora-do-catalogo.md) e a denylist nunca foi barreira (substring literal — `rm -r -f` e qualquer separador do `cmd.exe` passam) |
