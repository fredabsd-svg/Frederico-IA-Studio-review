...
| Filho faz `rm -rf /` no host | **dentro do sandbox: sim (afeta apenas workdir)** | `exec.python`/`exec.node` requerem aprovação por invocação (ADR-0034). No `exec.shell`, quem impede é a **allowlist** de 11 programas read-only ([ADR-0044](../decisions/0044-exec-shell-com-resolucao-propria-de-programa.md) §D3) — `rm` não resolve em programa nenhum. A denylist não conta como barreira: ela é substring literal (`rm -r -f` passa) e hoje é redundante por construção, mantida como tripwire |
| Ferramenta allowlisted executa binário plantado no workspace | não | **Caminho de fuga fechado no ADR-0044 §D1.** O `cmd.exe` busca o programa no diretório corrente antes do `PATH`, e o diretório corrente do filho é o workspace, onde o `files.write` escreve — plantar `findstr.bat` lá e pedir `findstr ...` executava o arquivo plantado. O `exec.shell` resolve o programa por caminho absoluto em `System32`, sem busca. Vale para qualquer ferramenta futura que deixe o `cmd.exe` resolver nome de programa: **não deixe** |
